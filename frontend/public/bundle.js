...
(function () {
  ...
  function isCategoryPassed() {
    return categoryNames$1.indexOf($('#mcqCategory').val()) > -1;
  }
  ...
  function createExplanationField(answerNumber, parentElement, required) {
    ...
    var answerResponse = $("<textarea></textarea>").addClass('md-textarea form-control explanation').prop('rows', '4').prop('required', required).prop('placeholder', 'Type what you would like to display when this answer is selected').prop('id', textAreaId);
  ...
  function checkMCQDuplicate(element) {
    ...
    $('#answers input').each(function check(i) {
      if (JSON.stringify(element) !== JSON.stringify($(this)) && element.val().toLowerCase().trim() === $(this).val().toLowerCase().trim()) {
        markInvalid($(this));
        markInvalid(element);
        $("#answer".concat(i, "Feedback")).text('Please enter a unique answer.');
  ...
  function addAnswerInputBoxButtonClick() {
    ...
    $("#answer".concat(numberOfAnswersCounter$1)).on('keyup focus', checkMCQAnswer);
    $("#answer".concat(numberOfAnswersCounter$1)).on('change', function checkDuplicate() {
      checkMCQDuplicate($("#answer".concat(numberOfAnswersCounter$1)));
    });
  }
